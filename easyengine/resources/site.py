...
class Site(Server):
    ...
    def validate_service(self, service):
        if (
            service not in self.service_site and
            service not in self.service_wordpress
        ):
    ...
    def create(self, domain, service=None):
        ...
        stdin, stdout, stderr = self.execute(
            'ee site create %s --%s' % (self._domain, self._service)
        )
        ...
        for line in lines:
            if 'Successfully created site' in line:
                url = line.replace(
                    '\x1b[94mSuccessfully created site ', ''
                ).replace('\x1b[0m\n', '')

                data.update({'url': url})
            elif 'WordPress admin user : ' in line:
                username = line.replace('\x1b[94m\x1b[0m', '').replace(
                    'WordPress admin user : ', ''
                ).replace('\x1b[0m\n', '')

                data.update({'username': username})
            elif 'WordPress admin user password : ' in line:
                password = line.replace('\x1b[94m\x1b[0m', '').replace(
    ...
    def update(self, domain, service=None, user=None, password=None):
        ...
        if hasattr(self, '_user') and hasattr(self, '_password'):
            stdin, stdout, stderr = self.execute(
                'ee site update %s --password' % self._domain
            )
    ...
    def generate_csr(
        ...
        command = (
            ...
            'openssl req -new -key %(domain)s.key -out %(domain)s.csr -sha256'
    ...
    def install_ssl(self, domain, certificate_code, ca_chain):
        ...
        # Write certificate
        command = (
            'echo "%(certificate_code)s" >>' +
            '%(path)s/cert/%(domain)s.crt') % dict(
        ...
        if len(errors):
            # Insert information
            command = (
                "sed -i '/www.%(domain)s;/a\    listen 80;" % dict(
                    domain=self._domain
                ) +
                "\\n    listen 443 ssl spdy;" +
                "\\n    ssl on;" +
                "\\n    ssl_certificate %(path)s/cert/%(domain)s.crt;" % dict(
                    path=self._path, domain=self._domain
                ) +
    ...
    def uninstall_ssl(self, domain):
        ...
        command = (
            "rm -rf %(path)s/cert/%(domain)s.crt" % dict(
                path=self._path, domain=self._domain
            )
        )
        stdin, stdout, stderr = self.execute(command)
